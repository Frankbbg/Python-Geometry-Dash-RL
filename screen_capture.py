...
def find_avatar(frame, saveImg=False, *, fileName=None):
    ...
    :param frame: The current game frame (screenshot).
    :return: A boolean indicating whether the avatar was found.
    """
    if frame.ndim == 3 and frame.shape[2] == 3:
        # Convert the frame to grayscale
        gray_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    else:
        # print("The frame is already grayscale or has an unexpected format.")
